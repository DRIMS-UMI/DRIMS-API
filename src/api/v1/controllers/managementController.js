...
// Deregister a student due to timeline exceeding or other reasons
export const deregisterStudent = async (req, res, next) => {
    try {
        const { studentId } = req.params;
        ...
        // Find the 'deregistered' status definition
        const deregisteredDef = await prisma.statusDefinition.findFirst({
            where: { name: 'deregistered' }
        });

        if (!deregisteredDef) {
            const error = new Error('Deregistered status definition not found in system. Please ensure it is created.');
            error.statusCode = 500;
            throw error;
        }
        ...
        await prisma.$transaction(async (tx) => {
            ...
            await tx.studentStatus.updateMany({
                ...
                data: { isCurrent: false, isActive: false, endDate: new Date() }
            });

            // Create the deregistered status entry
            await tx.studentStatus.create({
                data: {
                    student: { connect: { id: studentId } },
                    definition: { connect: { id: deregisteredDef.id } },
                    startDate: new Date(),
                    conditions: reason || 'Timeline exceeded/Policy deregistration',
                    isActive: true,
                    isCurrent: true,
                    updatedBy: req.user?.id ? { connect: { id: req.user.id } } : undefined
                }
            ...
            await tx.userActivity.create({
                data: {
                    ipAddress: req?.headers['x-client-ip'] || req?.ip || 'Unknown',
                    deviceId: req?.headers['x-device-id'] || 'Unknown',
                    browserAgent: req?.headers['user-agent'] || 'Unknown',
                    action: "Deregistered Student",
                    entityType: "Student",
                    entityId: studentId,
                    userId: req.user?.id,
...
// Reinstate a deregistered student with a new timeline (extension)
export const reinstateStudent = async (req, res, next) => {
    try {
        const { studentId } = req.params;
        const { newExpectedCompletionDate, reason } = req.body;

        if (!newExpectedCompletionDate) {
            const error = new Error('A new expected completion date (extension) is required for reinstatement');
            error.statusCode = 400;
            throw error;
        }
        ...
        // Find the 'deregistered' status definition to exclude it
        const deregisteredDef = await prisma.statusDefinition.findFirst({
            where: { name: { in: ['deregistered', 'DEREGISTERED'] } }
        });

        // Find the last status before deregistration to continue from there
        const lastStatus = await prisma.studentStatus.findFirst({
            where: {
                studentId,
                definitionId: { not: deregisteredDef?.id }
            },
            orderBy: { endDate: 'desc' },
            include: { definition: true }
        });

        // Find the 'WORKSHOP' status definition as a fallback
        const workshopDef = await prisma.statusDefinition.findFirst({
            where: {
                name: { in: ['WORKSHOP', 'workshop'] }
            }
        });

        const nextStatusId = lastStatus?.definitionId || workshopDef?.id;
        const nextStatusName = lastStatus?.definition?.name || 'WORKSHOP';

        if (!nextStatusId) {
            const error = new Error('No valid status phase found to reinstate the student into');
            error.statusCode = 500;
            throw error;
        }

        await prisma.$transaction(async (tx) => {
            // Deactivate the current (deregistered) status
            await tx.studentStatus.updateMany({
                where: { studentId, isCurrent: true },
                data: { isCurrent: false, isActive: false, endDate: new Date() }
            });

            // Create the new active status entry (continuing from the previous phase)
            await tx.studentStatus.create({
                data: {
                    student: { connect: { id: studentId } },
                    definition: { connect: { id: nextStatusId } },
                    startDate: new Date(),
                    conditions: reason || `Student reinstated to ${nextStatusName} after deregistration`,
                    isActive: true,
                    isCurrent: true,
                    updatedBy: req.user?.id ? { connect: { id: req.user.id } } : undefined
                }
            ...
            // Reactivate student and update their timeline
            await tx.student.update({
                where: { id: studentId },
                data: {
                    isActive: true,
                    currentStatus: nextStatusName.toUpperCase(),
                    expectedCompletionDate: new Date(newExpectedCompletionDate)
                }
            });
            ...
            await tx.userActivity.create({
                data: {
                    ...
                    deviceId: req?.headers['x-device-id'] || 'Unknown',
                    browserAgent: req?.headers['user-agent'] || 'Unknown',
                    action: "Reinstated Student",
                    entityType: "Student",
                    entityId: studentId,
                    userId: req.user?.id,
                    details: JSON.stringify({ reason, nextStatusName, newExpectedCompletionDate })
                }
            });
        }, { timeout: 15000 });

        res.status(200).json({
            message: `Student reinstated successfully to ${nextStatusName} phase`,
            studentId,
            nextStatusName,
...
export const generateFieldLetter = async (req, res, next) => {
    ...
    try {
        ...
        // Upload the PDF to GridFS instead of storing inline in MongoDB
        storedFileId = await storeFileToGridFS(pdfBuffer, {
            filename: `field-letter-${proposal.student.fullName}.pdf`,
            contentType: 'application/pdf',
            metadata: {
                proposalId,
                uploadedBy: req.user.id,
                documentType: 'FIELD_LETTER'
            }
        });
